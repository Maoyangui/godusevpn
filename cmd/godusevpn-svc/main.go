...
func main() {
	...
	case "uninstall":
		...
		// 服务对象删掉以后谁都拉不起它了,再撤一次闸、再还原一次网卡:上面撤闸到删服务之间,要是有人打开了客户端
		// (界面启动时服务停着就自动拉起)或点了「修复」,守护进程一起来就按"想连"把闸装回去、网卡 IPv6 再关掉,
		// 而删服务只停不撤 —— 接着程序和「恢复网络」被删,留下一道谁都撤不掉的闸。撤不掉就失败:卸载程序会中止、
		// 保留工具。
		if err := netmode.ClearGuard(); err != nil {
