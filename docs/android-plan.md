...
- 检查更新一直说"已是最新"(2026-09-08,v0.6.1-a5):版本比较在三段数字相同时只判断"有没有后缀",所以整个 0.6.0-aN 系列彼此都不算更新(a3 看不到 a4)。改成按"字母段 + 数字段"比后缀(a3 < a4,a9 < a10,正式版最大),并补了成套用例。
  已经装了 a0~a4 的客户端自身的比较逻辑是坏的,收不到修好的版本;所以下一版号从 0.6.0-a5 提到 **0.6.1-a5**,把补丁号抬高一位,老客户端按数字比较就能发现它,不用手动侧载。
- 第四轮(2026-09-08,v0.6.1-a7):
  - **Windows 一键导入点了没反应**:安装包从来没注册过 `godusevpn://` 协议(README 里写了、`[Registry]` 段里只有开机自启),所以浏览器根本不知道该拉起谁。补上 URL 协议的四条注册表项(HKA,管理员安装即 HKLM,卸载时删干净);客户端本身早就能接 `godusevpn://import?url=…`,装过新安装包才会生效。
  - **确认框样式与位置不对**:页面用的是浏览器自带的 confirm / prompt,Windows 的 WebView2 会画成「wails.localhost 显示」的系统弹窗贴在窗口左上角,安卓与浏览器面板也各带一行来源地址。改成应用内对话框(居中卡片、危险操作红色按钮、回车确认、Esc / 点背景 / 返回键取消,电视上也能用方向键选),五处调用(删订阅、删规则组、还原默认规则、设备改名、面板密码)全部换掉。
  - 协议注册补两道:安装包写机器级(HKLM),客户端每次启动再往 HKCU 写一份指向当前 exe —— 这样已经装了老版本的人不用重装,应用内升级后就能用;换了安装位置也会跟着更新。这段逻辑在本机 Windows 上有真实注册表用例覆盖(写入、读回、改路径、清理)。
