...
// Shutdown 服务停止 / 关机:停掉循环和内核,但**不动"想连"**。想不想连是用户的意愿(已落盘),不是服务的。
// 用 Disconnect 的话它先把想连清成假、再等状态机当前这一步跑完 —— 那一步要是正好在同步闸和网卡 IPv6
// (prepare、health),或者别处有调用正排队等锁,读到的"想连"就是假,于是按"用户断开"撤闸、还原 IPv6。
// 0.7.4 及以前的停服务就是这样;只在入口查一次"是否在停机"也堵不严(查完到读"想连"之间还有等锁的时间)。
